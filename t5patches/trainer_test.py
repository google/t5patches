...
TrainerRngDeterminismTest, and MutableTrainerTest in t5x.trainer_test.py.
A future refactor would be helpful in keeping the overall code DRY
but requires modifying the main t5x code, as a py_test cannot depend on another
py_test. A refactor would require moving shared code to a py_library that both
t5x.trainer_test.py and this file could depend on.
...
import numpy as np
from t5patches import models as models_lib
...
def fake_accum_grads(model, optimizer, batch, rng, num_microbatches,
                     data_partition_spec, orig_train_state):
...
def fake_eval_step(model, optimizer, batch, orig_train_state):
  del model, optimizer
...
def fake_eval_fn_without_weight_sum(params, batch, orig_params):
  del params
  # Add `i` to each metric.
  i = batch['i'].sum()
...
class SelfDistilledTrainerTest(parameterized.TestCase):
  ...
  def test_accumulate_grads_microbatched_without_weight_sum_single_batch(self):
    ...
    num_microbatches = 1
    grad_accum, metrics, flax_mutables = corrections_trainer_lib.accumulate_grads_microbatched(
  ...
  def test_accumulate_grads_microbatched_without_weight_sum_multiple_batches(
    ...
    grad_accum, metrics, flax_mutables = corrections_trainer_lib.accumulate_grads_microbatched(
        self.test_trainer._model, self.init_train_state, batch,
        self.test_trainer._base_rng, num_micro_batches, self.init_train_state)
  ...
  def test_eval_step_without_weight_sum(self):
    ...
    metrics = corrections_trainer_lib.eval_step(self.test_trainer._model,
...
class SelfDistilledTrainerRngDeterminismTest(parameterized.TestCase):
